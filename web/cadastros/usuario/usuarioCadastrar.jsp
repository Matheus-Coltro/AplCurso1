    ...
    $(document).ready(function () {
        ...
        $('#email').blur(function () {
            ...
            if (email === '') {
                Swal.fire({
                    icon: 'error',
                    title: 'O e-mail não pode ser vazio!'
                });
                return;
            }
            ...
            // Se passou no formato, verifica duplicidade no backend (só para inclusão)
            if ($('#id').val() == 0) {
                $.ajax({
                    type: 'get',
                    url: 'UsuarioVerificarEmail',  // servlet que você vai criar
                    data: { email: email },
                    success: function (response) {
                        if (response == '1') {
                            Swal.fire({
